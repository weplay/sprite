...
module Sprite::Magick
  class SpriteMagickError < RuntimeError; end
  
  class ImageTempFile < ::Tempfile
    def make_tmpname(ext, n)
      'sprite_tmp%d-%d%s' % [$$, n, ext ? ".#{ext}" : '']
    end
  end
    ...
    def format(format, page=0)
      ...
      unless File.exists?(@path)
        begin
          ...
          raise SpriteMagickError, "Unable to format to #{format}; #{e}" unless File.exist?(@path)
        end
      end
    ensure
  ...
  class Composite
    ...
    # image is in back). You will be returned a SpriteMagick::Image instance for the new composited image:
    #
    #   image1 = SpriteMagick::Image.open('foreground.png')
    #   image2 = SpriteMagick::Image.open('background.png')
    #   output_image = SpriteMagick::Composite.new(image1, image2, 'jpg', :gravity => 'NorthEast')
  ...
  class CommandRunner
    ...
    def self.run(command, *args)
      ...
      if $?.exitstatus != 0
        raise SpriteMagickError, "ImageMagick command (#{command.inspect}) failed: #{{:status_code => $?, :output => output}.inspect}"
      else
        output
      end
    end
